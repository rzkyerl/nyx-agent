<div align="center">
  ...
  <h1>NYX AGENT</h1>
  <p>An open-source AI chat assistant powered by multi-provider LLMs NVIDIA NIM, Groq, Google Gemini, and Ollama.</p>
...
- **Web search** automatically detects queries needing real-time information and fetches results via LangSearch or Serper; citations are shown inline as numbered sources
- **File uploads** read and analyze PDFs, Word documents, images, and spreadsheets directly in chat
...
You only need to configure the providers you want to use. At least one API key is required.
...
| `POST /api/export` | Generates a downloadable PDF, DOCX, or XLSX from a given content. |
...
| `lib/storage.ts` | Session persistence (localStorage) |
