...
Nyx Agent is a production-ready, open-source AI chat assistant built with **Next.js 16**, **React 19**, and the **Vercel AI SDK**. It supports multiple LLM providers with automatic fallback, streaming responses, web search, file uploads, and document generation, all in a clean, minimal UI. No database, no accounts. Everything lives in your browser.
...
- **Persistent memory** the AI summarizes facts about you after each exchange and injects them into future conversations; viewable and editable in Settings
...
- **Hallucination detection** warns you when the assistant's response contains phrases indicating outdated or missing information
...
| `GOOGLE_GEMINI_API_KEY` (alias: `GEMINI_API_KEY`) | One of these* | Google Gemini API key |
...
Any OpenAI-compatible endpoint can be added from the Composer's model selector:

1. Open the model selector -> Custom Providers
2. Enter a display name, base URL (e.g. `http://localhost:1234/v1`), and API key
3. Nyx Agent validates the endpoint via `/api/providers/validate` and fetches the model list
4. The provider and its models are stored in `localStorage` and available immediately

Custom providers can optionally be configured as **direct connection**, where requests go straight from the browser to the provider, bypassing the `/api/chat` proxy. This is useful for localhost endpoints but requires the provider to have CORS configured to accept browser requests.

### Skills

Skills are Markdown files with YAML frontmatter that inject custom instructions into the system prompt for specific tasks. You can install skills via the Composer's `+` menu or by typing `/` to trigger the slash command menu. Skills are stored in `localStorage` and activated by keyword matching against the user's message. Skills can also be packaged as a `.zip` archive containing a `skill.md` file for easy distribution.

### Persistent Memory

After each assistant reply, Nyx Agent calls `/api/memory` in the background to update a memory string, a compact summary of facts and preferences it has learned about you. This memory is injected into every future system prompt so the AI stays contextually aware across sessions. You can view, edit, or clear your memory in the Settings panel.

### Document Generation

When the AI response includes an `export-config` block, a download button appears automatically. The block specifies the file type (`pdf`, `docx`, `xlsx`), template style (`academic`, `formal`, `informal`), and font. The `/api/export` route renders the content and streams back the binary file. A live HTML preview panel (A4 simulation) is also available before downloading.
...
| `components/chat/composer.tsx` | Message input, file upload, model selector, skills |
...
| `components/chat/settings-panel.tsx` | Theme, memory, preferences, and custom provider management |
...
The `/api/chat` route is **public and unauthenticated**, so every request consumes your API key credits. Before deploying to production:
